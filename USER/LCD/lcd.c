	  ...
	  for(t=0;t<len;t++)
		{
		  if( t<(len-value_num))
			{
			  if(mode||t==(len-1))LCD_DisplayChar(x+(size/2)*t,y,'0',size);  
...
//color:这个汉字的颜色
//
void LCD_Show_CH_Font16(u16 x,u16 y,u8 index,u16 color,u16 bg_color)
{ 
	u8 temp,t,t1;
	u16 y0=y;
	for(t=0;t<32;t++)//每个16*16的汉字点阵 有32个字节
	{
		temp = hz16[index*32 + t];
		for(t1=0;t1<8;t1++)
			{
				if(temp&0x80)LCD_Color_DrawPoint(x,y,color);//画实心点
				else LCD_Color_DrawPoint(x,y,bg_color);//画空白点（使用背景色）
				temp<<=1;
				y++;
				if((y-y0)==16)
					{
...
//index:tfont数组里面的第几个汉字
//color:这个汉字的颜色
void LCD_Show_CH_Font24(u16 x,u16 y,u8 index,u16 color,u16 bg_color)
{
	u8 temp,t,t1;
	u16 y0=y;
	for(t=0;t<72;t++)//每个24*24的汉字点阵 有72个字节
		{
		temp = hz24[index*72 + t];
		for(t1=0;t1<8;t1++)
			{
				if(temp&0x80)LCD_Color_DrawPoint(x,y,color);//画实心点
				else LCD_Color_DrawPoint(x,y,bg_color);//画空白点（使用背景色）
