...
void KEY_Init(void)
{
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOF|RCC_AHB1Periph_GPIOA|RCC_AHB1Periph_GPIOC|RCC_AHB1Periph_GPIOD, ENABLE);    //使能GPIOF/C时钟
 
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_1|GPIO_Pin_6|GPIO_Pin_7|GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_12|GPIO_Pin_13|GPIO_Pin_14; //KEY0 KEY1 KEY2 KEY3对应引脚
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;             //普通输入模式
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;       //100M
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;             //上拉
  GPIO_Init(GPIOF, &GPIO_InitStructure);                   //初始化GPIOF6,7,8,9 0,1,12,13,14
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure);      
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5; //PC5对应引脚
  ...
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;                   //上拉
	GPIO_Init(GPIOF, &GPIO_InitStructure);                         //初始化GPIOF15
	
	GPIO_SetBits(GPIOF, GPIO_Pin_15);//F15
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11|GPIO_Pin_12|GPIO_Pin_13;
	GPIO_Init(GPIOD, &GPIO_InitStructure); //D11,12,13
  GPIO_SetBits(GPIOD, GPIO_Pin_11|GPIO_Pin_12|GPIO_Pin_13);
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;
	GPIO_Init(GPIOA, &GPIO_InitStructure); //C4
...
{	   
	...
	if(K_IN==0||KEY0==0||KEY1==0||KEY2==0||KEY3==0)   //有键正按下
	{
		if(K_IN==0)	key_tem=1;
		else if(KEY0==0) key_tem=2;
		else if(KEY1==0) key_tem=3;
		else if(KEY2==0) key_tem=4;
		else if(KEY3==0) key_tem=5;
